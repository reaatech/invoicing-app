...
const InvoiceList: React.FC = () => {
  ...
  const [editingInvoice, setEditingInvoice] = useState<{ [key: string]: string | number } | null>(
    null,
  );
  ...
  const loadInvoices = async () => {
    try {
      ...
      if (response.success) {
        setInvoices((response.data as { [key: string]: string | number }[]) || []);
      }
    } catch (error) {
  ...
  const filteredInvoices = invoices.filter((invoice) => {
    const matchesSearch = String(invoice.invoice_number)
      .toLowerCase()
  ...
  const handleSendInvoice = async (invoiceId: number, customerId: number | string) => {
    ...
    try {
      ...
      if (sendResponse.success) {
        ...
        toast.error(
          `Failed to send invoice: ${sendResponse.error || 'Please check SMTP settings and try again.'}`,
          {
      ...
      toast.error(`Failed to send invoice: ${message}`, {
        id: sendToastIdRef.current ?? undefined,
      });
    } finally {
  ...
  const handleSelectionChange = (selection: GridRowSelectionModel) => {
    const nextSelection = Array.isArray(selection) ? selection : Array.from(selection?.ids ?? []);
    ...
    <Paper sx={{ p: 3 }}>
                ...
                [
                  {
                    ...
                    renderCell: (params) => (
                      <Button
                        variant="text"
                        onClick={() => handleViewInvoice(Number(params.row.id))}
                      >
                        {params.value}
                      </Button>
                    ),
                  },
                  {
                    ...
                    renderCell: (params) => (
                      <Button
                        variant="text"
                        onClick={() => handleViewCustomer(params.row.customerId)}
                      >
                        {params.value}
                      </Button>
                    ),
      ...
      <Menu anchorEl={menuAnchorEl} open={Boolean(menuAnchorEl)} onClose={handleMenuClose}>
        ...
        <MenuItem
          ...
          disabled={
            !canEditInvoice(
              String(invoices.find((item) => Number(item.id) === Number(menuInvoiceId))?.status),
            )
          }
        >
        ...
        {['Draft', 'Sent', 'Overdue'].includes(
          ...
          >
            ...
            {sendingInvoiceId === Number(menuInvoiceId)
              ? 'Sending...'
              : String(
                    invoices.find((item) => Number(item.id) === Number(menuInvoiceId))?.status,
                  ) === 'Draft'
                ? 'Send'
                : 'Resend'}
          </MenuItem>
                ...
                api
                  ...
                  .then((response) => {
                    if (response.success) {
                      ...
                      toast.error(
                        'Failed to mark invoice as paid: ' + (response.error || 'Unknown error'),
                      );
                    }
                  })
        ...
        <MenuItem
          ...
          disabled={['Cancelled', 'Draft'].includes(
            String(invoices.find((item) => Number(item.id) === Number(menuInvoiceId))?.status),
          )}
        >
