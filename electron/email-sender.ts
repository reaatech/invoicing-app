...
export const registerInvoiceSend = () => {
  ...
  ipcMain.on('invoice-send', async (event, invoiceId, recipientEmail) => {
    ...
    try {
      ...
      // Fetch line items
      const lineItems = db
        .prepare('SELECT * FROM invoice_line_items WHERE invoice_id = ? ORDER BY sort_order')
        .all(invoiceId);

      console.info('[Invoice Send] Fetching invoice attachments');
      // Fetch attachments
      const attachments = db
        .prepare('SELECT * FROM invoice_attachments WHERE invoice_id = ?')
      ...
      const outputPath = path.join(
        invoicesDir,
        `${settings.invoice_prefix}-Invoice-${invoice.invoice_number}.pdf`,
      );
...
${settings.company_name}`,
          ...
          console.info('[Invoice Send] Email send completed', {
            durationMs: Date.now() - startTime,
          });
          console.info('[Invoice Send] Email sent');
          // Log email sent
          db.prepare(
            "INSERT INTO email_logs (invoice_id, recipient_email, sent_at, status, error_message) VALUES (?, ?, datetime('now'), 'success', NULL)",
          ).run(invoiceId, recipientEmail);
          ...
          retries--;
          console.error('[Invoice Send] Send attempt failed', {
          ...
          if (retries === 0) {
            ...
            db.prepare(
              "INSERT INTO email_logs (invoice_id, recipient_email, sent_at, status, error_message) VALUES (?, ?, datetime('now'), 'failed', ?)",
            ).run(invoiceId, recipientEmail, errorMessage);
