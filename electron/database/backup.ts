...
function getLastBackupDate(): string | null {
  const result = db.prepare('SELECT value FROM app_settings WHERE key = ?').get(LAST_BACKUP_KEY) as
    | { value: string }
    | undefined;
  ...
  if (checkBackupReminder()) {
    dialog
      .showMessageBox({
        ...
        message:
          'It has been a while since your last backup. Would you like to create a backup of your invoicing data now?',
        detail:
          'Regular backups protect against data loss. Backups are stored in a secure location.',
      })
      .then((response) => {
